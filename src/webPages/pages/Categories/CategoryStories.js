...
import {
  processGetStoryByGenre,
  processGetLoadMoreStoryByGenre,
...
export default function CategoryStories() {
  ...
  let genreStories = state.genreStories;
  console.log('genreStories', genreStories);
  let params = useParams();
  const [page, setPage] = useState(Number(1));

  let location = useLocation();
  let coverStory = location?.state;
  const getgenreStories = async () => {
    try {
      await processGetStoryByGenre(dispatch, params.genreId, page);
    } catch (error) {
  ...
  const loadMore = async () => {
    ...
    try {
      await processGetLoadMoreStoryByGenre(dispatch, params.genreId, page + 1);
    } catch (error) {
      notificationHandler('error', error.message);
    } finally {
      setIsLoadingPage(false);
  ...
  return (
    <WebLayout>
      <Helmet>
        <title>{`${coverStory?.name} stories on Whipik`}</title>
        <meta name='description' content={coverStory?.description} />
        <meta itemProp='image' content={coverStory?.description} />
        <meta property='og:description' content={coverStory?.description} />
        <meta property='og:image' content={coverStory?.image} />
        {/* H1 = {coverStory?.name} */}
      </Helmet>

      {coverStory && <CategoryHero coverStory={coverStory} />}
      <div className='max-w-6xl mx-auto px-4 pb-10'>
        <div className='pt-10 md:pt-20 mb-4'>
          <h2 className='text-3xl mb-2 font-bold flex items-center'>
            {coverStory && <span className='mr-2'>All {coverStory.name} Stories</span>}
            <svg
              width={24}
              height={28}
              viewBox='0 0 24 28'
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 my-6 sm:my-10 gap-4 sm:gap-10 pb-12'>
          {genreStories &&
            genreStories.stories.map(function (stories, index) {
              return <StoryListCard key={index} stories={stories} />;
            })}
        </div>
        <LoadMoreButton
          story_number={genreStories.stories}
          loadMore={loadMore}
          totalStories={genreStories}
        />
      </div>
    </WebLayout>
  );
