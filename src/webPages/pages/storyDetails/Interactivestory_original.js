...
export default function Stories({
  ...
  console.log('readingSettings outside', readingSettings);
  //scroll to bottom of chat messages
  const messageEl = useRef(null);
  useEffect(() => {
    console.log('readingSettings', readingSettings, messageEl);
    if (messageEl) {
  ...
  return (
    ...
    >
      <div className='backdrop-filter relative backdrop-blur-xl overflow-hidden px-4 sm:px-8 py-6 bg-black bg-opacity-75'>
        ...
        >
          {chapterMessages &&
            chapterMessages.chatMessages.slice(0, numberOfChatMessages).map(function (chat, index) {
              return (
                <MessageType
                  key={index}
