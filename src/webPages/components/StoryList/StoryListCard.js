...
export default function StoryListCard({ stories }) {
  return (
    <a href={`/story-detail/${stories._id}`}>
      <div className='flex hover:shadow-lg bg-white cursor-pointer rounded gap-4 md:gap-6 p-4 story-card-item items-center'>
        <div
          className='h-44 w-32 sm:w-36 rounded flex-none bg-cover text-center overflow-hidden story-card-img'
          style={{ backgroundImage: `url(${stories.coverImage})` }}
          title='Story cover image'
        ></div>
        <div className='flex flex-col story-card-desc'>
          <p className='mb-2 text-sm'>
            {stories?.author?.name} | {stories?.genre?.name}
          </p>
          <h1 className='mb-2 font-bold two_lines'>{stories.title}</h1>
          <p className='synopsis two_lines mb-3 text-sm'>{stories.synopsis}</p>
          <div className='flex text-sm text-gray-600 items-center w-52'>
